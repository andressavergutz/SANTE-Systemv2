...
OnOffApplication::OnOffApplication ()
  : m_socket (0),
    ...
    m_totBytes (0),
    m_timeTraffic (Simulator::Now().GetMilliSeconds())
{
...
void OnOffApplication::SendPacket ()
{
  ...
  // todo: alteração feita para add payload ao pacote com o tempo de envio
  //double m_timeTraffic = Simulator::Now().GetMilliSeconds();
  std::stringstream msgx;
  msgx << (m_timeTraffic);
  Ptr<Packet> packet = Create<Packet>((uint8_t*) msgx.str().c_str(), m_pktSize);
